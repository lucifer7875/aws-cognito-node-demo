...
router.post('/changePassword', async (req, res) => {
    ...
    if (body.email && body.oldPassword && body.newPassword && body.totpCode) {
        const { email, oldPassword, newPassword, totpCode } = body;

        try {
            await cognito.ChangePassword(email, oldPassword, newPassword, totpCode, (err, result) => {
                if (err) {
                    res.status(400).json({ error: err });
                } else {
                    res.status(200).json({ result });
